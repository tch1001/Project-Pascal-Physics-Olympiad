\documentclass{book}
\usepackage[left=0.75in,right=0.75in,top=1in,bottom=1in,bindingoffset=2in,marginparwidth=2in]{geometry}
\usepackage{lipsum}
\usepackage{graphicx}
...
\begin{titlepage}
\begin{center}
    {\huge\bfseries High School Physics Olympiad}
    \\[10pt]
 {\Large\bfseries Project Pascal}\\[5pt]
 \texttt{\url{https://projectpascal.com}}\\[14pt]
 For any erratas/clarifications, kindly Telegram \href{https://t.me/tch1001}{@tch1001}
\end{center}
\tableofcontents
\end{titlepage}
\reversemarginpar % required for images to appear on correct side of margin
\chapter{Kinematics 1}
...
This works if the graph is linear, i.e. $y=mx+c$. But what happens if the graph is not linear? e.g. $y = x^2$. How can we calculate the gradient of this curvy graph? Answer is \textbf{Differentiation}. \\[10pt]
...
\subsection{SUVAT}
\begin{align}
v&=u+a t \label{eq:vuat} \\
s&=u t+\frac{1}{2} a t^2 \label{eq:suat}\\
v^2&=u^2+2 a s \label{eq:vuas} \\
s&=\frac{v(t)+u}{2} t \label{eq:svut}\\
s&=v(t) t-\frac{1}{2} a t^2 \label{eq:svat}
\end{align}
...
\subsection{Exercises}
\begin{samepage}
\begin{exercisebox}{SJPO 2018 General Round Q13}
...
\includegraphics[width=\linewidth]{images/2018q10.png}\\
Ans: \ifpaper E \fi\\
\end{samepage}
...
\chapter{Translational Dynamics 1}
...
\begin{examplebox}[exampleboxfilled]{Inverse Square Force}
...
\end{align}
The left hand side of Equation \ref{eq:pepke} is actually potential energy + kinetic energy. We will talk about potential energy in future, but for now we observe that integrating $F$ with respect to $x$ instead of $t$ was a useful trick. 
\end{examplebox}

Generalizing this to a general force $F$,
...
Ans: \ifpaper D \fi
\end{examplebox}
\begin{examplebox}{SJPO 2016 General Round Q4}
...
\newpage
\chapter{Translational Dynamics 2}
\section{Systems with variable amounts of moving mass }
\section{Equations of motion in different coordinates (extension of the polar coordinate system taught in lesson 2) }
...
\newpage\clearpage 
\chapter{Accelerating Frames of Reference}

\chapter{Energy and Momentum 1}

\chapter{Energy and Momentum 2}
